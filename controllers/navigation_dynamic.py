...
def load_featured_products():
    ...
    for i in categories:
        nodes_str=""
        items_node_array.append(items.open_li()+items.a(href="{}/category/{}".format(settings.IP_SERVER,i.id),content="{} {}".format(i.name,items.span(span_class="caret"))))
...
def recurse(id,node_str):
    try:
        ...
        for i in sub:
            node_str.append(items.open_li()+items.a(href="{}/category/{}".format(settings.IP_SERVER,i.id),content="{} {}".format(i.name,items.span(span_class="caret"))))
